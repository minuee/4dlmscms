...
import IconTimeslice from 'imgs/player/icon_time_slice_fullscreen.svg';
...
function Timeslice() {
  ...
  useEffect(() => {  
    ...
    if(player) {
      player.setEventTarget('PC');
      if(direction) {         
        player.stopLoad();        
        player.setInitFrameSegment();
        player.setChannelDirection(direction);
        player.setMediaLiveOff(false);
        player.removeRemainingBuffer(false);
      } else {
        player.stopChannelSwitch();
      }
    }
  }, [playerContext.player, direction]);   
