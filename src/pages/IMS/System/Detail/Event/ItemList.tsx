...
import { useEventListRequest } from '@/apis/IMS/system/detail/event/list';

import { useAppSelector, useAppDispatch } from '@/redux/hooks';
import { ReducerType } from '@/redux/reducer';
import { ItemType } from '@/types/IMS/system/event/index';
...
import Item from '../../component/EventItem';
...
export const ItemList: React.FC<ItemListProps> = ({}) => {
  ...
  const handleChangePage = (page: number) => {
    // url 처리
    history.push(`${SYSTEM_UPDATE_EVENT}?page=${page}` );
    getListData(page);
  };
  ...
  const handleAddEvent = () => {
    ...
    history.push({ pathname: SYSTEM_UPDATE_EVENT_CREATE });
  };
              ...
              {list?.length !== 0 && (
                <thead>
                  <tr>
                    ...
                    <th className='th--basic--uppercase'>{t('list:contentId')}</th>
                    <th className='th--basic--uppercase'>{t('list:name')}</th>
                    <th className='th--basic--uppercase'>{t('list:description')}</th>
                    <th className='th--basic--uppercase'>{t('list:liveStatus')}</th>
                    <th className='th--basic--uppercase'>{t('list:isPublic')}</th>
                    <th className='th--basic--uppercase'>{t('list:status')}</th>
                    <th className='th--basic--uppercase'>{t('list:scheduledAt')}</th>
                    <th className='th--basic--uppercase'>{t('list:updatedAt')}</th>
                    <th className='th--basic--uppercase'>{t('list:registeredAt')}</th>
                  </tr>
                </thead>
              )}
